...
class AtosPayment
  ...
  def initialize(*args)
    ...
    @root_path = (paths[:root_path] ? paths[:root_path] : "#{Rails.root}/lib/atos")
    @request_path = (paths[:request_path] ? paths[:request_path] : "#{Rails.root}/lib/atos/bin/request")
    @response_path = (paths[:response_path] ? paths[:response_path] : "#{Rails.root}/lib/atos/bin/response")
    @pathfile_path = (paths[:pathfile_path] ? paths[:pathfile_path] : (paths[:banque] ? "#{Rails.root}/lib/atos/#{paths[:banque]}/pathfile" : "#{Rails.root}/lib/atos/sogenactif/pathfile"))
